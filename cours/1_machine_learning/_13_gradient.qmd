...
```{ojs}
//| echo: false
viewof epochG3 = Inputs.range([0, 100], {
  value: 0,
  step: 1,
  label: "Ajustement de la Presse (Époques)"
})
```
...
:::: {.card-body}
::::: {#d3-graph-3-container .w-100 .text-center}
:::::
::::
:::
...
Visualisez de manière interactive comment le paramètre d'inertie (Momentum) modifie le comportement de la descente dans un paysage de coût en trois dimensions.

::: {.card .card-window .mb-4}

:::: {.card-header}
🏔️ Simulateur 3D de la Descente de Gradient et Inertie (Momentum)
::::

:::: {.px-3 .pt-2 .pb-1 .text-muted .small}
Ajustez l'inertie (Momentum) pour voir comment la boule prend de la vitesse et de l'élan afin de descendre la surface en 3D représentant la fonction de coût, et observez si elle parvient à s'extraire des minima locaux.
::::
...
```{ojs}
//| echo: false
viewof momentumVal = Inputs.range([0.5, 1.0], {
  value: 0.771,
  step: 0.001,
...
//| echo: false
viewof triggerBtn = {
  const btn = html`<button class="btn btn-danger btn-sm d-flex align-items-center gap-2"><span>🔴</span> Lâcher la Balle</button>`;
  btn.value = 0;
  btn.onclick = () => {
    btn.value++;
...
```{ojs}
//| echo: false
import { runGradientSimulation } from "../../assets/js/custom/gradient-descent.js"

render3DDescent = {
  runGradientSimulation(momentumVal, triggerBtn, {
