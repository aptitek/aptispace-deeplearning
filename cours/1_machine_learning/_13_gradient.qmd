...
La mise à jour des poids à chaque étape : $w \leftarrow w - \eta \cdot \nabla_w \mathcal{L}$, où $\eta$ est le taux d'apprentissage et $\nabla_w \mathcal{L}$ le gradient.

**Adam** maintient deux moyennes mobiles — le gradient moyen ($m_t$) et le gradient carré moyen ($v_t$) — pour adapter le pas à chaque paramètre : $\hat{w} = w - \eta \cdot \frac{\hat{m}_t}{\sqrt{\hat{v}_t} + \epsilon}$.
:::
